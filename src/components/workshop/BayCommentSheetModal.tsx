...
interface BayCommentSheetModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  booking?: any;
  field?: any;
  workMode?: any;
  mode: "bay_submit" | "company_review" | "company_view";
  onSubmit?: (data: any) => void;
  onSuccess?: () => void;
  loading?: boolean;
}

const BayCommentSheetModal: React.FC<BayCommentSheetModalProps> = ({
  open,
  onOpenChange,
  booking: propBooking,
  field,
  ...
  // Fetch booking data for company_review and company_view modes
  const { data: fetchedBooking, isLoading: isFetchingBooking } = useQuery({
    queryKey: [
      "field-bay-booking-details",
      field?.vehicle_type,
  ...
  const booking = mode === "bay_submit" ? propBooking : fetchedBooking;
  const isLoading = mode === "bay_submit" ? false : isFetchingBooking;

  // Accept work mutation
  const acceptWorkMutation = useMutation({
    mutationFn: async () => {
      const response = await bayBookingServices.acceptWork(booking._id);
  ...
  const requestReworkMutation = useMutation({
    mutationFn: async (reason: string) => {
      const response = await bayBookingServices.requestRework(booking._id, reason);
      return response.data;
    },
    onSuccess: () => {
      toast.success("Rework request sent to bay user");
      setReworkReason("");
      onSuccess?.();
    },
  ...
  useEffect(() => {
    if (booking?.comment_sheet) {
      const sheet = booking.comment_sheet;
      setFormData({
        ...
        technician_company_assigned: sheet.technician_company_assigned || booking.accepted_by?.first_name + " " + booking.accepted_by?.last_name || "",
  ...
  const getQuoteDifference = () => {
    return 0; // Bay bookings don't have quote amounts
  };
  ...
  const getModalTitle = () => {
    switch (mode) {
      ...
      case "company_review":
        return "Review Bay Work Submission";
      case "company_view":
        return "Bay Work Details";
      default:
        return "Bay Work Details";
    }
  };
  ...
  const commonProps = {
    formData,
    setFormData,
    quote: booking, // Use booking as quote for compatibility with tabs
    mode: mode.replace("bay_", "supplier_") as any, // Map bay modes to supplier modes for tabs
    isReadOnly,
    uploading,
    setUploading,
    expandedEntry,
          ...
          <DialogTitle className="flex items-center gap-2 text-lg">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground font-normal">
                {booking?.field_name || field?.field_name} • Stock ID: {booking?.vehicle_stock_id || field?.vehicle_stock_id}
              </div>
            </div>
          </DialogTitle>
        </DialogHeader>
...
export default BayCommentSheetModal;
