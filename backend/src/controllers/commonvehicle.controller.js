...
// @desc    Get kanban report data for vehicles with workshop quotes
// @route   GET /api/common-vehicle/kanban-report
// @access  Private (Company Admin/Super Admin)
const getKanbanReport = async (req, res) => {
  try {
    ...
    if (
      ...
      baseFilter.dealership_id = { $in: dealershipObjectIds };
    }
    ...
    const vehicles = await VehicleModel.aggregate([
      { $match: baseFilter },
      {
        $lookup: {
          from: "workshopquotes",
          let: { vehicleStockId: "$vehicle_stock_id", vehicleType: "$vehicle_type" },
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [
                    { $eq: ["$vehicle_stock_id", "$$vehicleStockId"] },
    ...
    // Process vehicles to extract relevant data
    const processedVehicles = vehicles.map((vehicle) => {
      const registration = vehicle.vehicle_registration?.[0] || {};
      const source = vehicle.vehicle_source?.[0] || {};
      ...
      return {
        ...
        regoExpiry: registration.license_expiry_date,
        purchasePrice: source.purchase_price || otherDetails.purchase_price || 0,
        purchaseDate: source.purchase_date,
        workshop_quotes: vehicle.workshop_quotes || [],
      };
    });
    ...
    console.error("Get kanban report error:", error);
    res.status(500).json({
      success: false,
      message: "Error retrieving kanban report data",
    });
  }
};

// @desc    Update vehicle status (for kanban drag-and-drop)
// @route   PATCH /api/common-vehicle/:vehicleId/status
// @access  Private (Company Admin/Super Admin)
const updateVehicleStatus = async (req, res) => {
  try {
    const { vehicleId } = req.params;
    const { vehicle_type, status } = req.body;

    if (!vehicle_type || !status) {
      return res.status(400).json({
        success: false,
        message: "Vehicle type and status are required",
      });
    }
    ...
    await logEvent({
      ...
      event_description: `Vehicle ${vehicleId} status updated to ${status}`,
      user_id: req.user.id,
      company_id: req.user.company_id,
      user_role: req.user.role,
      metadata: {
