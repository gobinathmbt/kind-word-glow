/**
 * InspectionConfig Report Controller
 * Handles all inspection configuration analytics and reporting endpoints
...
/**
 * Get Inspection Config Usage
 * Analyzes inspection configuration usage patterns across the system
 * Includes active/inactive configs, default configurations, and version distribution
 * 
 * @route GET /api/company/reports/inspection-config/usage
...
const getInspectionConfigUsage = async (req, res) => {
  try {
    ...
    // 1. Get all inspection configs for the company
    const configs = await InspectionConfig.find(matchFilter)
    ...
    if (configs.length === 0) {
      return res.json(formatReportResponse({
        ...
        summary: {
          totalConfigs: 0,
          message: 'No inspection configurations found'
        }
      }, {
        reportType: 'inspection-config-usage'
      }));
    }
    ...
    return handleReportError(error, res, 'Inspection Config Usage');
  }
};

/**
 * Get Inspection Field Analysis
 ...
 * 
 * @route GET /api/company/reports/inspection-config/field-analysis
 * @access Private (company_super_admin, company_admin)
 */
const getInspectionFieldAnalysis = async (req, res) => {
  try {
    ...
    // 1. Get all inspection configs
    const configs = await InspectionConfig.find(matchFilter).lean();
    ...
    if (configs.length === 0) {
      return res.json(formatReportResponse({
        ...
        reportType: 'inspection-field-analysis'
      }));
    }
    ...
    return handleReportError(error, res, 'Inspection Field Analysis');
  }
};

/**
 * Get Inspection Category Effectiveness
 ...
 * 
 * @route GET /api/company/reports/inspection-config/category-effectiveness
 * @access Private (company_super_admin, company_admin)
 */
const getInspectionCategoryEffectiveness = async (req, res) => {
  try {
    ...
    // 1. Get all inspection configs
    const configs = await InspectionConfig.find(matchFilter)
      .populate('categories.master_dropdown_id', 'dropdown_name display_name')
      .lean();
    ...
    if (configs.length === 0) {
      return res.json(formatReportResponse({
        ...
        reportType: 'inspection-category-effectiveness'
      }));
    }
    ...
    configs.forEach(config => {
      ...
      categories.forEach(category => {
        ...
        if (category.master_dropdown_id) effectivenessScore += 10;
        ...
        allCategories.push({
          ...
          hasMasterDropdown: !!category.master_dropdown_id,
          masterDropdownName: category.master_dropdown_id?.dropdown_name,
    ...
    // 7. Analyze master dropdown usage
    const categoriesWithMasterDropdown = allCategories.filter(c => c.hasMasterDropdown);
    ...
    // 9. Categories needing improvement
    ...
    // 10. Section analysis across all categories
    ...
    const summaryStats = {
      ...
      avgCalculationsPerCategory: allCategories.length > 0
        ...
        : 0,
      categoriesWithMasterDropdown: categoriesWithMasterDropdown.length,
      masterDropdownUsagePercentage: allCategories.length > 0 
        ? Math.round((categoriesWithMasterDropdown.length / allCategories.length) * 100) 
        : 0,
      sectionsWithDescription: sectionsWithDescription.length,
    ...
    return handleReportError(error, res, 'Inspection Category Effectiveness');
  }
};
...
function getMissingFeatures(field) {
  ...
  if (!field.hasImage && ['text', 'dropdown'].includes(field.fieldType)) missing.push('image_support');
...
module.exports = {
  getInspectionConfigUsage,
  getInspectionFieldAnalysis,
  getInspectionCategoryEffectiveness
};
