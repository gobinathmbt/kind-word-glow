    ...
    socket.on("send_tender_message", async (data) => {
      try {
        ...
        try {
          ...
          if (socket.user.type === 'admin') {
            // Send to dealership users
            const dealershipUsers = await TenderDealershipUser.find({
              tenderDealership_id: dealership_id,
              isActive: true
            });

            const emailPromises = dealershipUsers.map(user => {
              const html = `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                  <h1 style="color: #3b82f6;">New Message for Tender ${tender.tender_id}</h1>
                  <p>Hello ${user.username},</p>
                  <p>You have received a new message regarding tender <strong>${tender.tender_id}</strong>:</p>
                  
                  <div style="background: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0;">
                    <p><strong>From:</strong> ${senderName}</p>
                    <p><strong>Message:</strong> ${content || 'File attachment'}</p>
                  </div>
                  
                  <p>Please log in to the dealership portal to view and respond to this message.</p>
              ...
              return mailService.sendEmail({
                to: user.email,
                subject: `New Message for Tender ${tender.tender_id}`,
                html
              });
            });
            ...
            // Send to admin user who created the tender
            const adminUser = await User.findById(tender.created_by);
            
            if (adminUser) {
              const html = `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                  <h1 style="color: #3b82f6;">New Message for Tender ${tender.tender_id}</h1>
                  <p>Hello ${adminUser.first_name},</p>
                  <p>You have received a new message from <strong>${dealership.dealership_name}</strong> regarding tender <strong>${tender.tender_id}</strong>:</p>
              ...
              await mailService.sendEmail({
                to: adminUser.email,
                subject: `New Message from ${dealership.dealership_name} - Tender ${tender.tender_id}`,
                html
              });
            }
          }
